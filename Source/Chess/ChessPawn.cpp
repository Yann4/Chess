...
void AChessPawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("ResetVR", EInputEvent::IE_Pressed, this, &AChessPawn::OnResetVR);
	PlayerInputComponent->BindAction("TriggerClick", EInputEvent::IE_Pressed, this, &AChessPawn::TriggerClick);
	PlayerInputComponent->BindAction("Cancel_Move", EInputEvent::IE_Pressed, this, &AChessPawn::CancelMove);
}
...
void AChessPawn::CancelMove()
{
	if (CurrentBlockFocus)
	{
		CurrentBlockFocus->OwningGrid->CancelMove();
	}
}
...
void AChessPawn::TraceForBlock(const FVector& Start, const FVector& End, bool bDrawDebugHelpers)
{
	...
	{
		AChessBlock* HitBlock = Cast<AChessBlock>(HitResult.Actor.Get());
